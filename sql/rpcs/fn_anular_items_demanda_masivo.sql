-- Propósito: Anulación masiva de ítems en demanda (requisiciones)
-- Última modificación: 2026-05-04

CREATE OR REPLACE FUNCTION public.fn_anular_items_demanda_masivo(p_ids_items bigint[], p_id_usuario bigint, p_motivo text)
 RETURNS jsonb
 LANGUAGE plpgsql
AS $function$
DECLARE
...
BEGIN
    ...
    -- 3. VERIFICACIÓN GLOBAL: ¿Queda algo por hacer en esta REQ (en cualquier almacén)?
    ...
    IF v_total_pendientes_global = 0 THEN
        ...
        SET estatus = 'PROCESADA', -- Se considera procesada porque ya no hay pendientes
            timestamp_update = v_now,
            timestamp_procesa = v_now, -- Marcamos como procesada (fin de ciclo)
