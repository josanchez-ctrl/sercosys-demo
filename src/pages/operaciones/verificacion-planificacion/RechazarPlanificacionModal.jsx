...
export default function RechazarPlanificacionModal({ plan, comedores, onClose, onConfirm, loading }) {
      ...
      <div className="bg-white w-auto h-auto max-w-md max-h-[98vh] rounded-[1.5rem] shadow-2xl overflow-hidden relative z-10 animate-in zoom-in-95 duration-300">
        ...
        <div className="p-8 pb-4 flex items-start justify-between">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h3 className="text-xl font-black text-slate-800 uppercase tracking-tighter">Rechazar Planificación</h3>
              <p className="text-xs text-slate-400 italic font-medium">Esta acción devolverá el plan a estatus BORRADOR</p>
            </div>
          </div>
        ...
        <div className="px-8 py-1 bg-gray-50/30">
          <div className="bg-white p-6 rounded-md border border-slate-100 shadow-sm space-y-4">
            ...
            <p className="text-xs text-slate-500 leading-relaxed border-t border-slate-50 pt-4 text-justify">
              Al rechazar esta planificación, el Gerente de Cocina podrá editarla nuevamente para realizar los ajustes necesarios antes de volver a enviarla a verificación.
            </p>
          </div>
        </div>
        ...
        <div className="sticky bottom-4 px-8 pt-4 flex items-center justify-end gap-3">
          ...
          <button
            disabled={loading}
            onClick={onConfirm}
            className="flex items-center gap-2 px-8 py-3 rounded-xl bg-red-500 text-white hover:bg-red-600 transition-all shadow-lg shadow-red-500/20 active:scale-95 disabled:opacity-50"
          >
            {loading ? <Loader2 size={16} className="animate-spin" /> : <XCircle size={16} />}
            <span className="text-[10px] font-black uppercase tracking-widest">Confirmar Rechazo</span>
          </button>
        </div>
      </div>
    </div>,
