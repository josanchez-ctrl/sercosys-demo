...
import { Utensils, Plus, Search, MoreHorizontal, Edit2, ToggleLeft, ToggleRight, Hash, Type } from 'lucide-react';
import { useModulePermissions } from '../../../hooks/useModulePermissions';
import { getTiposServicios, toggleEstatusTipoServicio } from '../../../services/tipoServicioService';
import TiposServiciosModal from './TiposServiciosModal';
import { supabase } from '../../../lib/supabase';
import ViewUser from '../../../components/user-table/ViewUser';
import { formatDateSystemToDDMMYYYY_HHMMSS } from '../../../util/workDate';

export default function GestionTiposServicios() {
  ...
  useEffect(() => {
    if (empresaActiva?.id) {
      ...
      const channel = supabase
        .channel('tipo_servicio_changes')
        .on('postgres_changes', { event: '*', schema: 'public', table: 'tipos_servicios_comida', filter: `id_empresa=eq.${empresaActiva.id}` }, () => fetchData())
        .subscribe();
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      const res = await getTiposServicios(empresaActiva.id);
      console.log(res);
  ...
  const handleToggleEstatus = async (item) => {
    try {
      await toggleEstatusTipoServicio(item.id, !item.estatus, perfil.id);
      fetchData();
    } catch (error) {
      alert(error.message);
    }
  ...
  const filteredData = tiposServicios.filter(t =>
  ...
  return (
    <div className="p-4 sm:p-8 space-y-8 animate-in fade-in duration-500 bg-slate-50 min-h-screen">
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-[10px] font-bold text-slate-400 uppercase tracking-[0.3em] mt-3">
              Configuración de servicios de alimentación
            </p>
          </div>
        </div>
      ...
      <div className="relative group/search max-w-xl">
        ...
        <input
          type="text"
          placeholder="Buscar servicio por nombre o abreviatura..."
          value={searchTerm}
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-gray-50/50">
                <th className="px-8 py-5 text-[10px] font-black text-slate-400 uppercase tracking-widest border-b border-gray-100">Orden / Nombre</th>
                ...
                <th className="px-8 py-5 text-[10px] font-black text-slate-400 uppercase tracking-widest border-b border-gray-100">Responsable</th>
            ...
            <tbody className="divide-y divide-gray-100">
              {loading ? (
                  ...
                  <tr key={item.id} className="hover:bg-slate-50/50 transition-all group/row">
                    <td className="px-8 py-6">
                      <div className="flex items-center gap-4">
                        ...
                        <span className="text-sm font-black text-slate-700 uppercase tracking-tight">{item.nombre}</span>
                      </div>
                    </td>
                    <td className="px-8 py-6">
                      <span className="text-[10px] font-black text-brand-900 bg-brand-50 px-3 py-1 rounded-lg uppercase tracking-widest">
                    ...
                    <td className="px-8 py-6 text-center">
                      <button
                        ...
                        className={`inline-flex items-center gap-2 px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-widest border transition-all ${item.estatus ? 'bg-emerald-50 text-emerald-600 border-emerald-200' : 'bg-red-50 text-red-600 border-red-200'}`}
                      >
                        {item.estatus ? <ToggleRight size={14} /> : <ToggleLeft size={14} />}
                    ...
                    <td className="px-8 py-6 text-right">
                      <button
                        ...
                        className="p-2 text-slate-300 hover:text-brand-900 hover:bg-brand-50 rounded-xl transition-all active:scale-90"
                      >
                        <Edit2 size={18} />
                      </button>
                    </td>
                ...
                <tr>
                  <td colSpan={5} className="px-8 py-20 text-center opacity-30 italic text-sm font-bold uppercase tracking-widest">No hay registros</td>
                </tr>
              )}
            </tbody>
          </table>
