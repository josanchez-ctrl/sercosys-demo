...
import { Warehouse, Search, Plus, FileEdit, CheckCircle2, XCircle, Trash2, Eye, ClipboardList, Clock, Calendar, Layers } from 'lucide-react';
import { useModulePermissions } from '../../../hooks/useModulePermissions';
import { getAlmacenes } from '../../../services/almacenService';
import { getOrdenesDespoteYReproceso, iniciarDespote, anularOrdenDespote } from '../../../services/despoteService';
...
import DespoteModal from './DespoteModal';
import ReprocesoModal from './ReprocesoModal';
...
export default function GestionDespote() {
  ...
  const [mostrarModalReprocesoNuevo, setMostrarModalReprocesoNuevo] = useState(false);
  ...
  useEffect(() => {
    if (empresaActiva?.id) {
      getAlmacenes(empresaActiva.id).then(async (data) => {
        // Filtrar el almacén frigorífico o permitir almacenes activos
        setAlmacenes(data || []);
        const now = await Now();
        if (now) {
  ...
  const fetchOrdenes = async () => {
    ...
    try {
      ...
      const data = await getOrdenesDespoteYReproceso(empresaActiva.id, dateStart, dateEnd);
      // Filtrar por almacén seleccionado si aplica
      if (almacenSel) {
      ...
      toast.error('Error al cargar órdenes de despote');
      console.error(e);
    } finally {
      setLoading(false);
    }
  ...
  const filteredOrdenes = useMemo(() => {
    return ordenes.filter(o => {
      ...
      // Filtro por término de búsqueda (ID, usuario o observaciones)
  ...
  const handleIniciar = (id) => {
    toast('¿Desea iniciar el proceso de despote?', {
      description: 'Esta acción descontará la canal/insumo de los racks y la trasladará a la mesa de trabajo.',
      duration: 8000,
      action: {
        label: 'Iniciar',
        onClick: async () => {
          setProcesandoId(id);
          try {
            await iniciarDespote(id, perfil.id);
            toast.success('Proceso de despote iniciado. Materia prima colocada en la mesa.');
            fetchOrdenes();
          } catch (e) {
            toast.error('Error al iniciar despote: ' + e.message);
            console.error(e);
          } finally {
  ...
  const handleAnular = async (id) => {
    toast('¿Está seguro de anular esta orden?', {
      description: 'Esta acción es irreversible y no se podrá procesar en el futuro.',
      duration: 8000,
      action: {
        label: 'Confirmar Anulación',
        onClick: async () => {
          try {
            await anularOrdenDespote(id, perfil.id);
            toast.success('Orden de despote anulada correctamente');
            fetchOrdenes();
          } catch (e) {
    ...
    <div className="p-4 space-y-2 animate-in fade-in duration-500 bg-slate-50">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-3xl font-black text-slate-800 uppercase tracking-tighter leading-none">Despote / Procesamiento</h1>
            <p className="text-sm text-slate-400 italic mt-1 uppercase tracking-widest font-bold">Deshuese y Porcionamiento de Cárnicos</p>
          </div>
        </div>
      ...
      {!almacenSel ? (
        <div className="flex flex-col items-center justify-center py-20 gap-6 bg-white rounded-[2.5rem] border border-slate-100 shadow-sm">
          ...
          <div className="text-center">
            ...
            <p className="text-sm text-slate-400 font-medium italic">Debe elegir un almacén activo (ej: Frigorífico) para gestionar sus órdenes de despote</p>
          </div>
        </div>
      ) : (
        <>
          <div className="flex flex-col md:flex-row items-start md:items-center gap-4 relative z-10 w-full">
            ...
            <div className="flex items-center gap-2">
              ...
              >
                ...
                <span className="text-[10px] font-black uppercase tracking-widest">Crear Despote</span>
              </button>

              {/* BOTÓN REGISTRAR NUEVO REPROCESO */}
              <button
                onClick={() => setMostrarModalReprocesoNuevo(true)}
                className="bg-orange-600 text-white rounded-md px-4 py-1.5 shadow-xl shadow-orange-600/10 hover:scale-105 active:scale-95 transition-all flex items-center justify-center gap-3 border border-orange-500 whitespace-nowrap"
              >
                <div className="p-2 bg-orange-500 rounded-xl"><Plus size={16} /></div>
                <span className="text-[10px] font-black uppercase tracking-widest">Crear Reproceso</span>
              </button>
            </div>
          </div>
          ...
          <div className="bg-white rounded-md shadow-xl shadow-slate-200/40 border border-slate-100 overflow-hidden">
            {loading ? (
                              ...
                              {est === 'BORRADOR' && (
                                <>
                                  ...
                                  <button
                                    ...
                                    title="Iniciar Despote (Cargar a Mesa)"
                                  >
                                    {procesandoId === o.id ? (
              ...
              <div className="flex flex-col items-center justify-center py-10 gap-6 bg-white">
                <div className="w-24 h-24 bg-slate-50 rounded-[2.5rem] flex items-center justify-center text-slate-300 animate-pulse">
                  <Layers size={48} />
                </div>
                <div className="text-center">
                  <p className="text-lg font-black text-slate-800 uppercase tracking-tighter">Sin órdenes de despote</p>
                  <p className="text-sm text-slate-400 font-medium italic">No se encontraron registros de despote en este rango de fechas</p>
                </div>
              </div>
            )}
          </div>
      ...
      {/* RENDER MODAL DESPOSTE */}
      {(mostrarModalNuevo || (ordenParaEditar && (ordenParaEditar.tipo_proceso === 'DESPOSTE' || !ordenParaEditar.tipo_proceso)) || (ordenParaVer && (ordenParaVer.tipo_proceso === 'DESPOSTE' || !ordenParaVer.tipo_proceso))) && (
        <DespoteModal
          ...
          ordenId={mostrarModalNuevo ? null : (ordenParaEditar?.id || ordenParaVer?.id)}
      ...
      {/* RENDER MODAL REPROCESO */}
      {(mostrarModalReprocesoNuevo || (ordenParaEditar && ordenParaEditar.tipo_proceso === 'REPROCESO') || (ordenParaVer && ordenParaVer.tipo_proceso === 'REPROCESO')) && (
        <ReprocesoModal
          empresaActiva={empresaActiva}
          sucursalActiva={sucursalActiva}
          perfil={perfil}
          almacenId={almacenSel?.id}
          nombreAlmacen={almacenSel?.nombre}
          ordenId={mostrarModalReprocesoNuevo ? null : (ordenParaEditar?.id || ordenParaVer?.id)}
          modoVisualizacion={!!ordenParaVer}
          onClose={() => {
            setMostrarModalReprocesoNuevo(false);
            setOrdenParaEditar(null);
            setOrdenParaVer(null);
          }}
          onUpdate={() => {
