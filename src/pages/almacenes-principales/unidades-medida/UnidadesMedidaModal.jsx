...
import { createUnidadMedida, updateUnidadMedida } from '../../../services/unidadesmedidaService';

const validationSchema = Yup.object({
    nombre: Yup.string().required('El nombre es obligatorio').min(2, 'Mínimo 2 caracteres'),
    abreviatura: Yup.string().required('Debe ingresar una abreviatura').min(1, 'Mínimo 1 caracter'),
    estatus: Yup.boolean(),
});

export default function CategoriaModal({ initialData = null, empresaActiva, perfil, onClose, onUpdate }) {
    const isEdit = !!initialData;

    const formik = useFormik({
        initialValues: {
            nombre: initialData?.nombre?.toUpperCase() || '',
            abreviatura: initialData?.abreviatura?.toUpperCase() || '',
        ...
        onSubmit: async (values) => {
            const payload = {
                ...
                abreviatura: values.abreviatura.trim().toUpperCase(),
            ...
            try {
                if (isEdit) {
                    await updateUnidadMedida(initialData.id, payload, perfil?.id);
                } else {
                    await createUnidadMedida(payload, perfil?.id);
                }
                onUpdate();
                onClose();
            } catch (err) {
                ...
                if (err.code === '23505') {
                    msg = 'Ya existe una unidad de medida con ese nombre en la empresa. Por favor, use un nombre diferente.';
                }
    ...
    return createPortal(
        ...
        >
            <div
                className="bg-white w-full sm:max-w-lg flex flex-col overflow-hidden rounded-[2.5rem] shadow-2xl animate-in zoom-in-95 duration-300 border border-white/20"
                    ...
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            ...
                            <p className="text-[11px] font-bold text-slate-400 uppercase tracking-widest mt-0.5 italic">
                                {isEdit ? initialData.nombre : 'Unidades de medida'}
                            </p>
                        </div>
                    </div>
                    ...
                    <div className="bg-white p-6 rounded-3xl border border-gray-200 shadow-sm space-y-6">
                        ...
                        <div className="space-y-2">
                            ...
                            <input
                                ...
                                placeholder="EJ: KILOGRAMO, LIBRA, LITRO, METRO..."
                            />
                        </div>
                        ...
                        {isEdit && (
                            <label className="flex items-center justify-between p-4 rounded-md bg-slate-50/50 border border-slate-100 cursor-pointer hover:bg-slate-100/50 transition-all group">
                                <div className="flex items-center gap-3">
                                    ...
                                    <span className="text-xs font-black text-slate-600 uppercase tracking-widest">Estado de Unidad de Medida</span>
                                </div>
