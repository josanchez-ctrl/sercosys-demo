...
import { Presentation, Plus, Search, Tag, Scale, Warehouse, Building2, Edit2 } from 'lucide-react';
...
export default function GestionPresentacion() {
    ...
    const fetchData = async () => {
        setLoading(true);
        try {
            const res = await getPresentaciones(empresaActiva.id);
            setData(res || []);
        } catch (error) {
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
                <div className="flex items-center gap-4">
                    <div className="w-14 h-14 rounded-[1.5rem] bg-brand-900 flex items-center justify-center text-white shadow-2xl shadow-brand-900/20 relative group overflow-hidden">
                        <Presentation size={28} className="relative z-10" />
                    ...
                    <div>
                        ...
                        <p className="text-[10px] font-bold text-slate-400 uppercase tracking-[0.3em] mt-3 flex items-center gap-2">
                            ...
                            Catálogo de Presentación <span className='font-black italic'>{empresaActiva?.nombre}</span>
                        </p>
                    </div>
                </div>
                    ...
                    <table className="w-full text-left border-collapse">
                        <thead>
                            <tr className="bg-gray-50/50">
                                <th className="px-8 py-5 text-[10px] font-black text-slate-400 uppercase tracking-widest border-b border-gray-100">Nombre de Presentación</th>
                                        ...
                                        <tr id={`group-${letter}`} className="bg-slate-50/90 backdrop-blur-sm sticky top-0 z-20">
                                            <td colSpan={3} className="px-8 py-2 border-y border-slate-100 shadow-sm">
                                                <div className="flex items-center gap-3">
                                                    ...
                                                    <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest bg-white px-3 py-1 rounded-full border border-slate-200 shadow-sm">
                                        ...
                                        {items.map((item) => (
                                            ...
                                            >
                                                <td className="px-8 py-4">
                                                    <div className="flex items-center gap-3">
                                                        <div className="p-2 bg-brand-50 text-brand-900 rounded-xl group-hover/row:bg-brand-900 group-hover/row:text-white transition-all">
                                                            <Tag size={16} />
                                                        </div>
                                ...
                                <tr>
                                    <td colSpan={3} className="px-8 py-20 text-center">
                                        <div className="flex flex-col items-center opacity-40">
                                            <Presentation size={48} className="mb-4 text-brand-900" />
                                            <p className="text-sm font-bold uppercase tracking-widest">No hay presentaciones registradas</p>
                                        </div>
                                    </td>
                                </tr>
                            )}
