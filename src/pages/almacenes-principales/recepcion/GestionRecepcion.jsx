...
import { PackagePlus, Plus, Search, Calendar, FileText, User, Warehouse, MoreHorizontal, CheckCircle2, XCircle, Clock, FileEdit, Package, AlertTriangle } from 'lucide-react';
...
export default function GestionRecepcion() {
    ...
    const filteredData = recepciones.filter(c => {
        const search = searchTerm.toLowerCase();
        // El Gerente solo ve PENDIENTE, PROCESADO y ANULADO
        const isTargetStatus = ['PENDIENTE', 'PROCESADO', 'ANULADO'].includes(c.estatus);
        ...
        return isTargetStatus && matchesStatus && (
            ...
            c.almacen_proveedores?.rif?.toLowerCase().includes(search)
        );
    });
    ...
    return (
        <div className="p-4 sm:p-8 space-y-2 animate-in fade-in duration-500 bg-slate-50 min-h-screen">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
                <div className="flex items-center gap-4">
                    <div className="w-14 h-14 rounded-[1.5rem] bg-brand-900 flex items-center justify-center text-white shadow-2xl shadow-brand-900/20 relative group overflow-hidden">
                        <PackagePlus size={28} className="relative z-10" />
            ...
            <div className="flex flex-col lg:flex-row items-start md:items-center gap-4 relative z-10">

                {/* Filtro de Semanas */}
                <div className="flex items-center gap-4 bg-white p-2 px-4 rounded-md border border-gray-100 shadow-sm animate-in slide-in-from-right duration-700 shrink-0">
            ...
            {(almacenSel && weekStart && weekEnd) && (
                <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
                    {Object.entries(statusConfig).map(([key, config]) => {
                        const count = recepciones.filter(c => c.estatus === key).length;
                    ...
                    <table className="w-full text-left border-collapse">
                        <thead>
                            <tr className="bg-gray-50/50">
                                <th className="px-8 py-5 text-[10px] font-black text-slate-400 uppercase tracking-widest border-b border-gray-100 whitespace-nowrap">Recepcion / Fecha / Estado</th>
                                        ...
                                        <tr key={cotejo.id} className="hover:bg-slate-50/50 transition-all group/row">
                                                ...
                                                <div className="flex flex-col gap-1.5">
                                                    <div className="flex items-center gap-2">
                                                        <span className="text-[11px] font-black text-slate-800 uppercase tracking-tight flex items-center gap-2">
                                                            <PackagePlus size={10} className="text-slate-400" />
                                            ...
                                            <td className="px-8 py-6">
                                                <div className="flex flex-col gap-1">
                                                    ...
                                                    <span className="whitespace-nowrap text-[9px] font-bold text-slate-400">{cotejo.almacen_proveedores?.rif}</span>
                                                </div>
                                            </td>
                                            <td className="px-8 py-6">
